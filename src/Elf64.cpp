...
int64_t Elf64::fetchSize(const ElfPart type) {
	...
	case ElfPart::ProgramHeaderTable:
		return _header.e_phnum * sizeof(elf64_phdr);
	case ElfPart::SectionHeaderTable:
		return _header.e_shnum * sizeof(elf64_shdr);
...
int64_t Elf64::fetchPosition(const ElfPart type) {
	switch (type) {
	case ElfPart::Header:
		return 0;
	case ElfPart::ProgramHeaderTable:
...
void Elf64::parsePHT(unsigned char* ptr) {
	_phdr = new elf64_phdr[_header.e_phnum];
	std::copy_n(ptr, sizeof(elf64_phdr) * _header.e_phnum, (unsigned char*)_phdr);
	if (_order == nativeMachineOrder()) {
		return;
	}
...
void Elf64::parseSHT(unsigned char* ptr) {
	_shdr = new elf64_shdr[_header.e_shnum];
	std::copy_n(ptr, sizeof(elf64_shdr) * _header.e_shnum, (unsigned char*)_shdr);
	if (_order == nativeMachineOrder()) {
		return;
	}
...
void Elf64::parseSymStr(unsigned char *ptr) {
...
std::string to_string(const elf64_shdr h, const char *str) {
	std::string ret{};
	int left = 0, right = 8;
...
std::string to_string(const elf64_shdr* h, const int len, const char *str) {
	std::string ret{};
	int left = 0, right = 8;
